...
const balanceSheetUrl = 'https://yh-finance.p.rapidapi.com/stock/v2/get-balance-sheet';

function getOptions(symbol: string, url: string) {
    return {
        method: 'GET',
        url,
        params: {symbol},
...
export const IntelligentTableRow = ({ticker}) => {
    ...
    const { isLoading: isStatsLoading, error: statsError, data: statsData } = useStats(ticker);
    const { isLoading: isBalanceSheetLoading, error: balanceSheetError, data: balanceSheetData } = useBalanceSheet(ticker);
    const isLoading = isStatsLoading || isBalanceSheetLoading;

    if(isLoading || !statsData || !balanceSheetData) return <TransitionText text={"Loading"} />;
    if(statsError || balanceSheetError) return <TransitionText text={"Error"} />;
    
    return (
        <>
            {statsData && balanceSheetData && <IntelligentTableRowUI data={statsData} balanceSheetData={balanceSheetData} />}
        </>
    );
}